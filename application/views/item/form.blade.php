<div class='d3-tooltip'/>
	...
	<div class='item'>
		...
		@if($item->stats['speed'])
			{{ round($item->stats['speed'], 2) }} <span class='stat-helper'>Attacks per Second</span>
		@elseif($item->stats['block-chance'])
			{{ $item->stats['block-chance']}} <span class='stat-helper'>Chance to Block</span>
		@endif
		</p>
		<p class='stats stats-extra-range'>
		@if($item->stats['damage'])
			{{ $item->stats['damage']['min'] }} - {{ $item->stats['damage']['max'] }} <span class='stat-helper'>Damage</span>
		@elseif($item->stats['block-amount'])
			{{ $item->stats['block-amount']['min'] }}-{{ $item->stats['block-amount']['max'] }} <span class='stat-helper'>Block Amount</span>
		@endif			
		</p>
		<ul class='attrs'>
